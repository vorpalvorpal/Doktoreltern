 ...
 * top-level `*.md` files — never `turn.xml`, never a nested doc (so the
 * parked docs under `archive/` are deliberately outside the loop). Thread
 * ids are workspace-global, so id allocation scans every doc this returns
 * plus the `threads/` store.
 */
