...
name: implement
description: >
  Orchestrate implementation of an approved plan for a data-engineering R project by
...
own the data-integrity judgement: benchmarks, tests, correctness review, and the
...
**Default to sequential.** Pipeline stages usually build on each other.
...
part**: objective, data contract (schema/business rule + source), the exact
...
- **Match the model to the task.** Mechanical, verbose-but-easy stages →
  start cheap (haiku/sonnet). **Stages touching joins, keys, or business rules → never start on
...
Read the diff yourself — completeness, **data correctness** (does it
implement the cited schema/business rule, with rows accounted for?), scope (no stray files), conflicts with
...
- **Minor** (formatting, typos, namespacing, a simple bug, missing docs) →
  ...
  draft.
- **Major** (wrong implementation, misread rule, wrong approach) → **discard
...
- **Major or data-contract** change → tell the user to return to **`/plan`**, and
...
3. **Correctness review is yours and is non-negotiable. NEVER rubber-stamp
   subagent output** — read the diff and check the data handling yourself, every
