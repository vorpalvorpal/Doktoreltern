...
description: >
  Benchmark and optimise data-pipeline R code without changing its results. Use
...
# Benchmarking and optimising data-pipeline R code

The prime directive from the `conventions` skill holds here above all:
**correctness first.** A faster pipeline that mangles rows is still broken. This skill covers
...
1. **Algorithm / complexity.** The biggest, safest wins. An O(n log n) method
   ...
   source as you would any non-obvious business rule.
...
4. **Better data structures / backends.** Push work into the database —
   DuckDB/arrow/`dbplyr` — rather than pulling data into R; `data.table` for large
...
- Over-engineering for a 1% gain at the cost of readability or clarity of the
  pipeline.
- Trading correctness for speed, or changing output data under the banner of
  "optimisation" — that's a deferred contract decision, not a free win.
